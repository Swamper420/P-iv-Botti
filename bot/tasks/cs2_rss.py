...
from bot.config import BotConfig, Cs2RssConfig
...
class Cs2RssNotifier:
    def __init__(self, config: BotConfig | Cs2RssConfig) -> None:
        if isinstance(config, Cs2RssConfig):
            self._rss_config = config
            self._storage_dir = None
        else:
            self._rss_config = config.cs2_rss
    ...
    async def _run(self, app: Application) -> None:
        while True:
            ...
            await asyncio.sleep(self._rss_config.poll_interval_seconds)

    async def _fetch_rss(self) -> str | None:
        def _fetch() -> str:
            with urlopen(
                self._rss_config.url,
                timeout=self._rss_config.request_timeout_seconds,
            ) as response:
    ...
    async def check_updates(self, app: Application) -> None:
        ...
        storage_dir = getattr(self._config, "storage_dir", self._storage_dir)
...
def register(application: Application, config: BotConfig) -> None:
    notifier = Cs2RssNotifier(config)
