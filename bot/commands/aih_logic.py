...
def parse_aih_command(
    text: str, default_tokens: int = 100, max_tokens: int = 2000
) -> tuple[bool, int, str]:
    """Parse an !aih command string into (is_match, token_count, prompt)."""
    if not text:
        return False, default_tokens, ""

    match = re.match(r"(?i)^\s*!?aih(\d+)?(?::?\s*|\s+)?(.*)$", text, re.DOTALL)
    if not match:
        return False, default_tokens, ""
    ...
    if digits_str:
        ...
        # Check if the prompt starts with a number (e.g. "!aih 400: prompt" or "!aih 400 prompt")
...
async def stream_ollama_completion(
    ...
    """Stream text completion chunks from an Ollama instance using /api/generate."""
