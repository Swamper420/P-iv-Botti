...
from bot.commands.thug_logic import compose_thug_image
from bot.config import BotConfig

COMMAND_USAGE = "!thug [kuva tai vastaus kuvaan]"

LOGGER = logging.getLogger(__name__)
_THUG_REGEX = r"(?i)^\s*!thug\s*$"
...
def _extract_target_photo(message: object) -> PhotoSize | None:
    ...
    if photos and isinstance(caption, str):
        match = re.match(_THUG_REGEX, caption)
        if match:
            return photos[-1]
    ...
    if isinstance(text, str) and reply_photos:
        match = re.match(_THUG_REGEX, text)
        if match:
...
def _build_handler(
    ...
    async def handle_thug(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        processed = await asyncio.to_thread(
            compose_thug_image,
            photo_bytes,
            assets_dir=config.storage_dir / "assets",
        ...
        if not processed:
            LOGGER.error("!thug image processing failed")
            await reply_in_chunks(
                update,
        ...
        await message.reply_photo(photo=InputFile(BytesIO(processed), filename="thug.png"))
...
def register(application: Application, config: BotConfig) -> None:
    application.add_handler(
        MessageHandler(
            (filters.PHOTO & filters.CaptionRegex(_THUG_REGEX))
            | (filters.REPLY & filters.Regex(_THUG_REGEX)),
